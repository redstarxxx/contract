...
// 配置区 - 从环境变量获取配置
const CONTRACT_FILENAME = process.env.REACT_APP_CONTRACT_FILENAME;
...
if (!CONTRACT_FILENAME) {
  throw new Error("环境变量 REACT_APP_CONTRACT_FILENAME 未设置");
}
if (!contractAddress) {
  throw new Error("环境变量 REACT_APP_CONTRACT_ADDRESS 未设置");
}
if (!RPC_URL) {
  throw new Error("环境变量 REACT_APP_RPC_URL 未设置");
}
if (!FAUCET_URL) {
  throw new Error("环境变量 REACT_APP_FAUCET_URL 未设置");
}

// 检查私钥
const rawKey = process.env.REACT_APP_PRIVATE_KEY;
if (!rawKey) {
  throw new Error("环境变量 REACT_APP_PRIVATE_KEY 未设置");
}
...
if (!/^https?:\/\//.test(FAUCET_URL)) {
  throw new Error("无效的水龙头 URL 格式");
}

// 导入合约 ABI
const noteArtifact = require(`.././contracts/${CONTRACT_FILENAME}.sol/${CONTRACT_FILENAME}.json`);
const abi = noteArtifact.abi;
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      if (err.code === 'INSUFFICIENT_FUNDS') {
        errorMessage = `账户余额不足，请从水龙头获取测试代币: ${FAUCET_URL}`;
      }
      setNotification({
        type: 'error',
        message: '提交失败！',
